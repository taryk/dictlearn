...
sub new {
    ...
    $self->hbox_dst_item([]);
    $self->vbox_dst->Add($self->hbox_add, 0, wxALIGN_LEFT | wxLEFT | wxTOP,
        5);
    ...
    ### btn
    $self->btn_add(
        Wx::Button->new($self, wxID_ANY, 'Add Example', wxDefaultPosition));
    $self->btn_tran(
        Wx::Button->new($self, wxID_ANY, 'Translate', wxDefaultPosition));
    $self->btn_clear(
        Wx::Button->new($self, wxID_ANY, 'Clear', wxDefaultPosition));
    $self->btn_cancel(
        Wx::Button->new($self, wxID_ANY, 'Cancel', wxDefaultPosition));

    # layout
    $self->hbox_btn(Wx::BoxSizer->new(wxHORIZONTAL));
    $self->hbox_btn->Add($self->btn_add, 0, wxBOTTOM | wxALIGN_LEFT | wxLEFT,
        5);
    ...
    EVT_SELECTED($self, $self->btn_addexisting,
        sub { $self->add_existing_item(@_) });
...
sub make_dst_item {
    ...
    EVT_BUTTON(
        $self,
        $self->text_dst->[$id]{btnm},
        sub { $self->del_dst_item($id); }
    );
    $self->hbox_dst_item->[$id]
        ->Add($self->text_dst->[$id]{text}, 4, wxALL | wxEXPAND, 0);
    $self->hbox_dst_item->[$id]
        ->Add($self->text_dst->[$id]{vbox}, 1, wxALL, 0);
    $self->text_dst->[$id]{vbox}
        ->Add($self->text_dst->[$id]{btnm}, 1, wxALL, 0);
    ...
    if ($ro) {
        ...
        $self->text_dst->[$id]{edit}
            = Wx::Button->new($self, wxID_ANY, 'e', wxDefaultPosition);
        EVT_BUTTON(
            $self,
            $self->text_dst->[$id]{edit},
            sub { $self->edit_example_as_new($id) }
        );
        $self->text_dst->[$id]{vbox}
            ->Add($self->text_dst->[$id]{edit}, 1, wxALL, 0);
    }
...
sub add_dst_item {
    ...
    $self->vbox_dst->Insert($#children || 0,
        $el->{parent_hbox}, 1, wxALL | wxGROW, 5);
    $self->Layout();
    $el;
}
...
sub edit_example_as_new {
    ...
    $self->text_dst->[$example_id]{vbox}
        ->Remove($self->text_dst->[$example_id]{edit});
...
sub add {
    my $self = shift;
    return
        unless $self->text_src->GetValue
        or $self->text_src->GetValue !~ /^\s+$/;
    my %params = (
...
sub load_example {
    my $self    = shift;
    my %params  = @_;
    my $example = $main::ioc->lookup('db')->schema->resultset('Example')
        ->select_one($params{example_id});
    my @translate;
...
sub load_words {
    ...
    for my $item (
        $main::ioc->lookup('db')->schema->resultset('Word')->select(
            Dict::Learn::Dictionary->curr->{language_orig_id}{language_id}
        )
        )
    {
...
sub translate {
    ...
    if (keys %$res > 1) {
        ...
        for my $partofspeach (keys %$res) {
            ...
            $self->word_dst->[$i][0]
                ->SetSelection($self->get_partofspeach_index($partofspeach));
            $self->word_dst->[$i][1]->SetValue(
                join ' | ' => map { ref $_ eq 'ARRAY' ? $_->[0] : $_ }
                    @{$res->{$partofspeach}});
            $i++;
        }
    }
