...
sub new {
    ...
    $self->hbox_words->Add($self->l_word, 0,
    ...
    $self->hbox_res->Add($self->res, 0, wxRIGHT | wxTOP | wxLEFT | wxEXPAND,
        5);
    ...
    $self->btn_prev(
        Wx::Button->new(
            $self, wxID_ANY, 'Prev', wxDefaultPosition, wxDefaultSize
        )
    );
    $self->btn_next(
        Wx::Button->new(
            $self, wxID_ANY, 'Next', wxDefaultPosition, wxDefaultSize
        )
    );
    $self->btn_reset(
        Wx::Button->new(
            $self, wxID_ANY, 'Reset', wxDefaultPosition, wxDefaultSize
        )
    );
