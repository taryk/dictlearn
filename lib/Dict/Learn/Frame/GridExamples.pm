...
sub new {
    ...
    $self->hbox_btn->Add($self->btn_delete_item, 0,
        wxBOTTOM | wxALIGN_LEFT | wxLEFT, 5);
    $self->hbox_btn->Add($self->btn_clear_all, 0,
        wxBOTTOM | wxALIGN_LEFT | wxLEFT, 5);
    $self->hbox_btn->Add($self->btn_refresh, 0,
        wxBOTTOM | wxALIGN_LEFT | wxLEFT, 5);
...
sub update_examples {
    ...
    for ($obj->GetCol()) {
        when (COL_EXAMPLE->[0]) {
            $upd_example{text}
                = $self->grid->GetCellValue($obj->GetRow(), COL_EXAMPLE->[0]);
        }
        when (COL_INTEST->[0]) {
            $upd_example{in_test}
...
sub select_examples {
    ...
    for my $item (@items) {
        ...
        $self->grid->SetCellValue($i, COL_EXAMPLE->[0],
            $item->{COL_EXAMPLE->[1]});
