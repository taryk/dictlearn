...
sub new {
  ...
  ### table
  $self->lb_result( Wx::ListCtrl->new( $self, wxID_ANY, wxDefaultPosition, [350, 300], wxLC_REPORT|wxLC_HRULES|wxLC_VRULES ) );
  ...
  $self->lb_result->InsertColumn( 2 , 'Past Participle', wxLIST_FORMAT_LEFT, 100);
  $self->lb_result->InsertColumn( 3 , 'Score',           wxLIST_FORMAT_LEFT, 40);
  $self->l_top(Wx::StaticText->new($self, wxID_ANY, 'Here are your answers: ', wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT));
  $self->l_question(Wx::StaticText->new($self, wxID_ANY, 'Do you want to store the results?', wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT));

  ### result
  $self->l_correct( Wx::StaticText->new($self, wxID_ANY, 'Correct: 0', wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT) );
  $self->l_wrong(   Wx::StaticText->new($self, wxID_ANY, 'Wrong: 0',   wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT) );
  $self->l_total(   Wx::StaticText->new($self, wxID_ANY, 'Total: 0',   wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT) );
  # layout
  ...
  ### buttons
  $self->btn_ok( Wx::Button->new($self, wxID_OK, 'OK', wxDefaultPosition, wxDefaultSize) );
  $self->btn_cancel( Wx::Button->new($self, wxID_CANCEL, 'Cancel', wxDefaultPosition, wxDefaultSize) );
  # layout
...
sub fill_result {
  ...
  $self->l_total->SetLabel(sprintf 'Total: %.1d%%', $res->{correct}*100/$count);
  $self
}
