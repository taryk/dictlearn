...
has spin => (
  ...
  isa => 'Wx::SpinCtrl',
  default => sub {
    Wx::SpinCtrl->new($_[0], wxID_ANY, 0, wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS|wxSP_WRAP)
  }
);
...
has text => (
  ...
  isa     => 'Wx::StaticText',
  default => sub {
    Wx::StaticText->new($_[0], wxID_ANY, '', wxDefaultPosition, wxDefaultSize, wxALIGN_CENTRE)
  }
);
...
has input => (
  ...
  isa     => 'Wx::TextCtrl',
  default => sub {
    Wx::TextCtrl->new($_[0], wxID_ANY, '', wxDefaultPosition, wxDefaultSize, wxTE_LEFT)
  }
);
...
has btn_prev => (
  ...
  isa     => 'Wx::Button',
  default => sub {
    Wx::Button->new($_[0], wxID_ANY, 'Prev',  wxDefaultPosition, wxDefaultSize)
  }
);
...
has btn_next => (
  ...
  isa     => 'Wx::Button',
  default => sub {
    Wx::Button->new($_[0], wxID_ANY, 'Next',  wxDefaultPosition, wxDefaultSize)
  }
);
...
has btn_reset => (
  ...
  isa     => 'Wx::Button',
  default => sub {
    Wx::Button->new($_[0], wxID_ANY, 'Reset',  wxDefaultPosition, wxDefaultSize)
  }
);
...
sub _build_result {
  Dict::Learn::Frame::TranslationTest::Result->new(
    $_[0], wxID_ANY, 'Result', wxDefaultPosition, [800, 600],
...
{ my @ids;
  ...
  sub load_exercise_ids {
    my ($self) = @_;
    # let's get all ids
    my $id_rs = $main::ioc->lookup('db')->schema->resultset('Word')->search(
      { 'me.in_test' => 1,
        'word2_id.in_test' => 1,
        'me.lang_id' => Dict::Learn::Dictionary->curr->{language_orig_id}{language_id},
      },
      {select => ['word_id'],
  ...
  sub fetch_exercises {
    ...
    my $words = $main::ioc->lookup('db')->schema->resultset('Word')->search(
      { 'me.word_id'  => { in => [ @ids[ $from .. $to ] ] } },
      { distinct => 1 }
    );
...
sub next_step {
  ...
  if ($self->pos >= $self->max) {
    ...
    for my $i (0..$self->max - 1) {
      ...
      my $compare_res = $self->compare_strings($e_item->[2], $e_item->[3]);
      push @res, {
        word_id => $e_item->[0],
        word    => $e_item->[1],
        note    => $compare_res->[2],
    ...
    if ($self->result->fill_result(@res)->ShowModal() == wxID_OK) {
      ...
      $main::ioc->lookup('db')->schema->resultset('TestSession')->add(
        TEST_ID,
        sum(map { $_->{score} } @res),
        \@res
      );
...
sub compare_strings {
  ...
  if (length $got > 0) {
    for my $expected_item (@{ $expected }) {
      ...
      my $plus = (reduce { $a + $b } map { length $_ } ($a[0] =~ /\[([^\]]+)\]/g)) // 0;
      my $minus = (reduce { $a + $b } map { length $_ } ($a[0] =~ /\{([^\}]+)\}/g)) // 0;
      my $total_perc
          = 100 - ($plus > $minus
          ? $plus / length($got_stripped)
          : $minus / length($string_stripped)) * 100;
  ...
  my $matched_most = ( @e == 1 ? $e[0] : reduce { $a->[0] > $b->[0] ? $a : $b } @e );
  push @{ $matched_most }, grep { $matched_most->[2] ne $_->[1] } @{ $expected };
  $matched_most
}
