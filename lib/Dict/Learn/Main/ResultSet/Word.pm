...
sub find_ones {
    ...
    if ($params{word} && scalar @{$params{word}} > 0) {
        # Words with a relevance of 100% should go first
        for my $i (0 .. $#result) {
            next if $result[$i]->{word_orig} ne $params{word}[0];
            push @first_elements, delete $result[$i];
        }
    }
    return (@first_elements, @result);
}
